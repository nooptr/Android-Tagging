...
public class TaggingActivity extends Activity implements OnItemClickListener {
	...
	ArrayList<String> mSuggestedTags = new ArrayList<String>();
...
//		mSuggestedTags = intent.getStringArrayListExtra(EXTRA_SUGGESTED_TAGS);
		...
		mFadeInAnimation = AnimationUtils.loadAnimation(this,
				R.anim.tag_item_fadein);
		...
		mTagLayout.setAreaHint(mPickOnly ? R.string.picker_hint
				: R.string.tagarea_hint);
	...
	private void init() {
		...
		int addTagButton = a.getResourceId(
				R.styleable.TagTheme_addButtonBackground,
				R.drawable.black_button);
		int addTagColor = a.getColor(R.styleable.TagTheme_addButtonTextColor,
				0xffffffff);
	...
	public class TagListAdapter extends BaseAdapter implements
			OnTagChangeListener {
		...
		public TagListAdapter(Activity context, List<String> tags) {
			...
			TypedArray a = getTheme().obtainStyledAttributes(
					R.styleable.TagTheme);
			mItem = a.getResourceId(R.styleable.TagTheme_tagListItem,
					R.layout.tag_row);
		}
